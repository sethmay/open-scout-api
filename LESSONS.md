...
- **Before believing an audit, audit the auditor.** A corpus sweep that reports thousands of defects is far more likely to be broken than the corpus is: 949 of the first 1,377 findings here were the tool's own — a read cap that truncated PDFs into unparseability, a `.docx` scored as XML noise, and site navigation counted as page evidence. The tell was the flag *rate* on things every Scout camp has: 17% of camps supposedly claiming archery with no trace of it is not a data problem, it is a matcher problem. Sanity-check a finding rate against what the world is actually like before acting on a single row.
- **Never let a fetch failure masquerade as evidence of absence.** Caching an unreadable source as an empty page silently converts "we could not look" into "the page says nothing", which an audit then reports as a defect. Unreadable evidence must be its own bucket, and anything depending on it is *unverified*, never *unsupported*.

...
| 8 | **Merit badge popularity by year** | ✅ `merit-badge-ranking` | 🌱 **SEEDED (0.47.0):** 5 years (2021-2025), 692 badge-year ranks, every year a complete 1..N, from On Scouting's annual recap. ⚠ **Renamed from "earned-counts": Scouting America publishes RANKS ONLY** — no absolute number appears in any post — so `metric` is `earned_rank` and the schema rejects `earned_count`. Prior-year columns recover 2021 and corroborate 2022/2024; one 2022 typo corrected from the 2023 restatement. Follow-ups: pre-2021 years (older posts exist and were not walked); **the three 2025 badges (Artificial Intelligence, Cybersecurity, Multisport) carry `valid_from: null` in `data/merit-badges/` although the rankings evidence they were new in 2025 — the source's 999 sentinel implies a first-offered year worth confirming from a primary source.** | On Scouting (onscouting.org) annual merit badge rankings |

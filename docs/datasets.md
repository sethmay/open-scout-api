...
| [Awards](#awards) | 52 | 52 | Earned awards and recognitions — knots, honors, training awards |
| [Positions of responsibility](#positions-of-responsibility) | 29 | 29 | Youth leadership positions that satisfy rank requirements |
| [Adult training](#adult-training) | 28 + 67 | 28 + — | Courses, and what each position must complete |
| [Merit badge popularity](#merit-badge-popularity) | 5 years, 692 ranks | — | Badge popularity 2021–2025, as ranks |
| [Controlled vocabularies](#controlled-vocabularies) | 7 lists, 172 terms | — | Every coded value with a label and a description |

`—` means the dataset has no `current` projection: a rankings year and a training rule are not
...
Endpoints: `v1/current/councils.json`, `v1/councils/index.json`, `v1/councils/{id}.json` —
...
evergreen prose — the dataset most people arrive for.
...
- Each version carries the badge's name, its introduction year (`valid_from`), its retirement year
  ...
  `eagle_required`.
- **61 `superseded` events chain the renames** and 67 `discontinued` events close the rest — 128
  events for 128 historical badges, so nothing retires without a record. Lineages are walkable:
  `clerk → business → american-business`, `mining → rocks-and-minerals → geology`,
...
`v1/requirement-sets/index.json` lists all 667. That split is deliberate — see
...
- `audience` is `youth` on all 29. Adult leadership roles are a separate catalog — see
...
- 68 (rule, BSA position code) pairs over 45 distinct codes, and **194 course links, 86 of them
  alternatives** — a rule can be satisfied more than one way.
...
post of its own — it is recovered from the prior-year column the 2022 post publishes, which is
...
query like "has aquatics" resolve the whole subtree —

...
| **Camps** | ✅ 487 entities — imported from [camp-finder](https://github.com/sethmay/camp-finder), classified by `camp_type` (376 resident, 69 day, 42 high-adventure) and `operator` (483 council + 4 national bases: Philmont, Florida Sea Base, Northern Tier, Summit/James C. Justice). Most carry an evergreen `summary` (405 of 487; durable prose, no dates/fees/sessions); 29 sub-camps are grouped under a reservation `parent`. Coordinates carry `geo_precision` (387 `exact`, 99 `approximate` city/state backfill, 1 unplaceable) and a build gate rejects any point outside its state box. Covers every camp the camp-finder site lists; sessions/fees stay at the council site. |
